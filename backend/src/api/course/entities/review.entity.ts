...
export class Review extends Entity<ReviewProps> {
  ...
  static create(props: Omit<ReviewProps, 'createdAt'> & { createdAt?: Date }, id?: UniqueId): Review {
    if (props.rating < 1 || props.rating > 5) {
      throw new DomainException(ErrorCode.VALIDATION_ERROR, 'Rating must be between 1 and 5');
    }
    return new Review({
      ...props,
